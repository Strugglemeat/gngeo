...
static inline void RENAME(draw)(unsigned int tileno, int sx, int sy, int zx, int zy,
      ...
      if(yflip) {
        ...
        for(y = 0; y < zy; y++) {
          ...
          if(gfxdata[1] || gfxdata[0]) {
            ...
            col = (myword >> 0) & 0xf;
            if(col) {
              PUTPIXEL(br[0], paldata[col]);
            }
            col = (myword >> 4) & 0xf;
            if(col) {
              PUTPIXEL(br[1], paldata[col]);
            }
            col = (myword >> 8) & 0xf;
            if(col) {
              PUTPIXEL(br[2], paldata[col]);
            }
            col = (myword >> 12) & 0xf;
            if(col) {
              PUTPIXEL(br[3], paldata[col]);
            }
            col = (myword >> 16) & 0xf;
            if(col) {
              PUTPIXEL(br[4], paldata[col]);
            }
            col = (myword >> 20) & 0xf;
            if(col) {
              PUTPIXEL(br[5], paldata[col]);
            }
            col = (myword >> 24) & 0xf;
            if(col) {
              PUTPIXEL(br[6], paldata[col]);
            }
            col = (myword >> 28) & 0xf;
            if(col) {
              PUTPIXEL(br[7], paldata[col]);
            }
            myword = gfxdata[0];
            col = (myword >> 0) & 0xf;
            if(col) {
              PUTPIXEL(br[8], paldata[col]);
            }
            col = (myword >> 4) & 0xf;
            if(col) {
              PUTPIXEL(br[9], paldata[col]);
            }
            col = (myword >> 8) & 0xf;
            if(col) {
              PUTPIXEL(br[10], paldata[col]);
            }
            col = (myword >> 12) & 0xf;
            if(col) {
              PUTPIXEL(br[11], paldata[col]);
            }
            col = (myword >> 16) & 0xf;
            if(col) {
              PUTPIXEL(br[12], paldata[col]);
            }
            col = (myword >> 20) & 0xf;
            if(col) {
              PUTPIXEL(br[13], paldata[col]);
            }
            col = (myword >> 24) & 0xf;
            if(col) {
              PUTPIXEL(br[14], paldata[col]);
            }
            col = (myword >> 28) & 0xf;
            if(col) {
              PUTPIXEL(br[15], paldata[col]);
            }
          }
#ifdef DEBUG_VIDEO
          br -= 544;
    ...
    if(xflip) {
      ...
      if(yflip) {
        ...
        for(y = 0; y < zy; y++) {
          ...
          if(dda_x_skip[ 3]) {
            if((col = ((myword >> 12) & 0xf))) {
              PUTPIXEL(*br, paldata[col]);
            }
            br++;
          }
          if(dda_x_skip[ 4]) {
            if((col = ((myword >> 16) & 0xf))) {
              PUTPIXEL(*br, paldata[col]);
            }
            br++;
          }
          if(dda_x_skip[ 5]) {
            if((col = ((myword >> 20) & 0xf))) {
              PUTPIXEL(*br, paldata[col]);
            }
            br++;
          }
          if(dda_x_skip[ 6]) {
            if((col = ((myword >> 24) & 0xf))) {
              PUTPIXEL(*br, paldata[col]);
            }
            br++;
          }
          if(dda_x_skip[ 7]) {
            if((col = ((myword >> 28) & 0xf))) {
              PUTPIXEL(*br, paldata[col]);
            }
            br++;
          }
